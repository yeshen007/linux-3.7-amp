...
{
	if (resume) {
		printk("__enable_irq resume %d\n", irq);
		if (!(desc->istate & IRQS_SUSPENDED)) {
			printk("__enable_irq !IRQS_SUSPENDED\n");
			if (!desc->action)
				return;
...
__setup_irq(unsigned int irq, struct irq_desc *desc, struct irqaction *new)
{
	...
	/* 在gic_irq_domain_map中设置为其他的，其他控制器的应该也一样
    ...
    */
	...
	if (old) {
		...
		do {
			 ...
			 * new action.
			 */
			thread_mask |= old->thread_mask; //thread_mask累积所有action的thread_mask
			old_ptr = &old->next;
			old = *old_ptr;
		} while (old);
		shared = 1;		/* 标记这是一个共享中断 */
			 ...
			 const char *devname, void *dev_id)
{
	 ...
	 * logic etc).
	 */
	if ((irqflags & IRQF_SHARED) && !dev_id)	 /* 如果是共享中断则dev_id不能为null */
		return -EINVAL;
