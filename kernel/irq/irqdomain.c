					 ...
					 void *host_data)
{
	...
	*/
	for (i = 0; i < size; i++) {
		 ...
		 */
		if (ops->map)
			ops->map(domain, irq, hwirq);	/* 调用gic_irq_domain_map设置desc的流控层和irq_chip， 
												这里没有映射硬件中断号和虚拟中断号，在前面legacy方法映射，
												在4.0版本内核是在gic_irq_domain_map中映射 		*/
 ...
 */
 //#define pr_debug(fmt,arg...)			printk(fmt,##arg) //yejc
