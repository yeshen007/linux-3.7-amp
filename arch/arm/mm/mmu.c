...
static void __init devicemaps_init(struct machine_desc *mdesc)
{
	 ...
	 */
	vectors = early_alloc(PAGE_SIZE); 	/* 分配一页的内存，返回映射后得到的虚拟地址 */

	early_trap_init(vectors);		/* 将异常向量表和处理函数拷贝到vectors */
...
void __init paging_init(struct machine_desc *mdesc)
{
	...
	devicemaps_init(mdesc);		/* 映射device io区，在amp中这里还映射amp相关区域 */
