...
/* 将在l2中的物理地址start到end之间的内容失效，但不处理bm */
...
/*  */	
	 ...
	 * detect if bm context is in the inv_range
         */
	if((start < AMPPHY_START) && (end > (AMPPHY_START + BM_CONTEXT_SIZE))) {	//start-end包含bm区域全部
		end1 = AMPPHY_START;	//bm start
		start1 = AMPPHY_START + BM_CONTEXT_SIZE;		//bm end

		raw_spin_lock_irqsave(&l2x0_lock, flags);
        if (start & (CACHE_LINE_SIZE - 1)) {	//如果start不是行对齐
                start &= ~(CACHE_LINE_SIZE - 1);	//将start行对齐
                debug_writel(0x03);
                l2x0_flush_line(start);	//刷掉这一行(先clean再失效)
                debug_writel(0x00);
                start += CACHE_LINE_SIZE;	//start指向下一行
        }
        ...
        while (start < end1) {		//将start到end1之间的失效掉
        ...
        while (start1 < end) {		//将start1到end之间的失效掉
