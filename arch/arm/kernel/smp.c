...
int __cpuinit boot_secondary(unsigned int cpu, struct task_struct *idle)
{
	if (smp_ops.smp_boot_secondary)
		return smp_ops.smp_boot_secondary(cpu, idle);	//socfpga_boot_secondary
	return -ENOSYS;
}
...
asmlinkage void __cpuinit secondary_start_kernel(void)
{
	...
	__asm__ __volatile__("mrc p15, 0, %0, c1, c0, 1" : "=&r" (cpu) : : "cc");
	printk("cpu1 p15 aux reg:0x%08x\n", cpu);
	__asm__ __volatile__("mrc p15, 0, %0, c1, c0, 0" : "=&r" (cpu) : : "cc");
	printk("cpu1 p15 ctr reg:0x%08x\n", cpu);
